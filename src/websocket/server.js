...
export function setupLocalWebsocketServer(sockets, database, port, host, registry) {
  ...
  localServer.on("connection", (socket, req) => {
    ...
    socket.on("message", function (data) {
      onMessage(sockets, socket, data, database, host, registry);
    });
  });
  console.log("setupLocalWebsocketServer", localServer);
  return localServer;
}
